...
namespace fafo {
	GameStateLockedTileTutorialSelect::GameStateLockedTileTutorialSelect(MainGame* main_game) : m_main_game{ main_game } {};

	void GameStateLockedTileTutorialSelect::on_enter() {
	...
	void GameStateLockedTileTutorialSelect::select_square(NumberSquare number_square) {
	...
	void GameStateLockedTileTutorialSelect::enter_merge_state() {
		m_main_game->update_state(std::make_unique<GameStateMerge>(m_main_game));
	}

	void GameStateLockedTileTutorialSelect::on_exit() {
		m_main_game->set_responsive(false);
		m_main_game->disable_boost_buttons();
		m_main_game->set_tutorial_complete(true);
	};

	std::unique_ptr<GameState> GameStateLockedTileTutorialSelect::preprocess_state() {
		return nullptr;
	};
	void GameStateLockedTileTutorialSelect::update(float dt) {};
